...
//                }
//            }
            TagCloudView(tags: day.feelings)
        }
            ...
            ForEach(self.tags, id: \.self) { tag in
                self.item(for: tag.rawValue)
                    ...
                    .alignmentGuide(.leading, computeValue: { d in
                        if (abs(width - d.width) + 15 > g.size.width)
                        {
                            width = 0
                            height -= d.height
                        }
...
struct SummaryDayMoodModalView_Previews: PreviewProvider {
    static var previews: some View {
        SummaryDayMoodModalView(day: Day(date: Date(), mood: .happy, answers: ["aaaa", "aaaaaaa", "aaa"], feelings: [.angry, .anxious, .confident,.insecure, .loving, .proud]))
    }
}
